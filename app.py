...
class Main(tk.Frame):
    ...
    def init_main(self): # Метод со всеми графическими элементами(поля,кнопки)
        ...
        toolbar = tk.Frame(bg="#d7d8e1", bd=2)              # Установили цвет фона рамки и ширину рамки
        toolbar.pack(side=tk.TOP, fill=tk.X)                # Размещаем в окне
        # self.add_img = tk.PhotoImage(file="img/img.png")  # Загрузили изображение кнопки добавление в переменнную
        self.add_img = Image.open("img/img.png")
        self.add_img_format = self.add_img.resize((60, 40))
        self.add_img_finish = ImageTk.PhotoImage(self.add_img_format)
        # Создали кнопку
        ...
        btn_open_dialog = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.add_img_finish, command=self.open_dialog
        )
        ...
        self.update_img = Image.open("img/update.png")
        self.update_format = self.update_img.resize((60, 40))
        self.update_img_finish = ImageTk.PhotoImage(self.update_format)
        # Создали кнопку
        # 1- привязали к панели инструментов
        # 2- установили цвет фона
        # 3- установили размер рамки
        ...
        btn_edit_dialog = tk.Button(
            ...
            bg="#d7d8e1",
            bd=0,
            image=self.update_img_finish,
            command=self.open_update_dialog,
        )
        btn_edit_dialog.pack(side=tk.LEFT)                          # Размещение кнопку в окне, указывавая гду она(кнопка) будет находиться
        self.delete_img = Image.open("img/delete.png")
        self.delete_format = self.delete_img.resize((60, 40))
        self.delete_img_finish = ImageTk.PhotoImage(self.delete_format)
        ...
        btn_delete = tk.Button(
            ...
            bg="#d7d8e1",
            bd=0,
            image=self.delete_img_finish,
            command=self.delete_records,
        )
        btn_delete.pack(side=tk.LEFT)                               # Размещение кнопку в окне, указывавая гду она(кнопка) будет находиться
        self.search_img = Image.open("img/search.png")
        self.search_format = self.search_img.resize((60, 40))
        self.search_img_finish = ImageTk.PhotoImage(self.search_format)
        # Загрузили изображение кнопки обновления в переменнную
        # Создали кнопку
        # 1 - привязали к панели инструментов
        # 2- установили цвет фона
    ...
    def view_records(self):
        self.db.cursor.execute("SELECT * FROM users")                                      #Запрос: Выбираем все данные из таблицы
    ...
    def update_records(self, name, tel, email, salary):
        self.db.cursor.execute(                                                         # Запрос на обновление
            """UPDATE users SET name=?, tel=?, email=?, salary=? WHERE id=?""",
    ...
    def delete_records(self):
        for selection_items in self.tree.selection():
            self.db.cursor.execute(                                                     # Запрос на удаление строки с таким-то id
                "DELETE FROM users WHERE id=?", (self.tree.set(selection_items, "#1"))     # Передаём аргумет на место "?"
            )
        self.db.conn.commit()                                                           # Сохраняем запрос
        self.view_records()                                                             # Вызываем функцию класса
    ...
    def search_records(self, name):
        ...
        self.db.cursor.execute("SELECT * FROM users WHERE name LIKE ?", (name,))           # Сюда передаем кортеж (name), а не просто name
        [self.tree.delete(i) for i in self.tree.get_children()]                         # Старые данные удаляем
        [self.tree.insert("", "end", values=row) for row in self.db.cursor.fetchall()]  # Вписываем новые данные в бд
...
class DB:
    ...
    def data(self):
        insert_into = 'INSERT INTO users (name, tel, email, salary) VALUES (?, ?, ?, ?)'
        user_data=('Ivan', '+1298875776655', 'ivan@gmail.com', '120')
        user_data1=('Boris', '+72398873123655', 'boris@gmail.com', '130')
        user_data2=('Ruslan', '+167988657776655', 'rus@yandex.com', '140')
        user_data3=('Fedor', '+798568543545454', 'fedor@gmail.ru', '150')
        user_data4=('Eugene', '892312343451232', 'eugene@mail.com', '160')
    ...
    def insert_data(self, name, tel, email, salary): # Метод для добавленияя наших данных в таблицу
        self.cursor.execute(                                                                                        # Запрос на создание бд
            """INSERT INTO users(name, tel, email, salary) VALUES(?, ?, ?, ?)""", (name, tel, email, salary)    # передаём аргументы на места '?'
        )
        self.conn.commit()                                                                                          # Сохраняем запрос
